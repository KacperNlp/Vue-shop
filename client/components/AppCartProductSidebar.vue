<template>
  <div class="flex justify-between flex-col flex-nowrap gap-2 relative">
    <div class="flex justify-between gap-2">
      <img :src="$imgUrl(thumbnail)" :alt="name" class="w-20" />
      <div class="text-right">
        ...
        <button @click="handleClickRemoveProductFromCart" class="mt-1">
          <Icon name="ph:trash" width="24" height="24" class="text-gray-600 hover:text-rose-600" />
        </button>
      </div>
    </div>
    <div class="w-8">
      <el-input-number v-model="currentQuantity" :min="1" :max="99" @change="handleUpdateProductQuantity" />
    </div>
  </div>
</template>
